...
all: fmt lint vendor | $(BASE) ; $(info $(M) building executable…) @ ## Build program binary
	$Q cd $(BASE) && $(GO) build \
		-tags release \
		-ldflags '-X $(PACKAGE)/vendor/github.com/prometheus/common/version.Version=$(VERSION) -X $(PACKAGE)/vendor/github.com/prometheus/common/version.Revision=$(GITREVISION) -X $(PACKAGE)/vendor/github.com/prometheus/common/version.BuildDate=$(DATE) -X $(PACKAGE)/vendor/github.com/prometheus/common/version.BuildUser=$(USER) -X $(PACKAGE)/vendor/github.com/prometheus/common/version.Branch=$(GITBRANCH)' \
